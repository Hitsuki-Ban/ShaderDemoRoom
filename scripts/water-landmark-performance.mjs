...
const [outputPath] = process.argv.slice(2).filter((argument) => argument !== '--');
if (missingEnvironment.length > 0 || process.argv.slice(2).filter((argument) => argument !== '--').length !== 1) {
  throw new Error(
...
const ROOM_ID = 'voxel-water';
const WARMUP_SECONDS = 5;
const MEASUREMENT_SECONDS = 15;
const PAIR_COUNT = 5;
const MINIMUM_PAIRED_SPEED_RATIO = 0.9;
const CANDIDATE_DRAW_CALL_BUDGET = 20;
const BASELINE_DRAW_CALL_BUDGET = 19;
...
function roomUrl(baseUrl) {
  return `${baseUrl.replace(/\/+$/, '')}/#/room/${ROOM_ID}`;
}

async function measurePage(browser, baseUrl, label, drawCallBudget) {
  ...
  try {
    ...
    const diagnostics = await page.evaluate(() => {
      ...
      const telemetryJson = document.querySelector('[data-telemetry-json]')?.getAttribute('data-telemetry-json');
      return {
    ...
    if (!Number.isFinite(drawCalls) || drawCalls <= 0 || drawCalls > drawCallBudget) {
      throw new Error(`${label} draw calls ${drawCalls} exceeded budget ${drawCallBudget}.`);
    }
    return { label, renderer: diagnostics.renderer, drawCalls, ...cadence };
  } finally {
    await page.close();
  }
}
...
try {
  for (let pair = 1; pair <= PAIR_COUNT; pair += 1) {
    ...
    const first = candidateFirst
      ? await measurePage(browser, candidateUrl, 'candidate', CANDIDATE_DRAW_CALL_BUDGET)
      : await measurePage(browser, baselineUrl, 'baseline', BASELINE_DRAW_CALL_BUDGET);
...
const report = {
  ...
  comparison: 'T-VW-04 deployed baseline vs T-VW-01 landmark candidate',
  room: { id: ROOM_ID, preset: 'default' },
  ...
  drawCallBudgets: {
    baseline: BASELINE_DRAW_CALL_BUDGET,
    candidate: CANDIDATE_DRAW_CALL_BUDGET,
  },
  pairs,
};

await writeFile(outputPath, `${JSON.stringify(report, null, 2)}\n`);
console.log(`water landmark performance: ${outputPath}`);
console.log(
