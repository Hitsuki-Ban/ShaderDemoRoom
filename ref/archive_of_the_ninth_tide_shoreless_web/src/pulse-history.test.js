...
const DESKTOP_CAPACITIES = Object.freeze({ systemCapacity: 5, userCapacity: 3 });
const MOBILE_CAPACITIES = Object.freeze({ systemCapacity: 2, userCapacity: 2 });
...
describe('pulse history rings', () => {
  it('uses the fixed desktop and mobile capacities', () => {
    const desktop = createPulseHistory(DESKTOP_CAPACITIES);
    const mobile = createPulseHistory(MOBILE_CAPACITIES);
  ...
  it('shares the system ring between auto and system while keeping user isolated', () => {
    const history = createPulseHistory(DESKTOP_CAPACITIES);
  ...
  it('overwrites only the oldest member of each mobile ring', () => {
    const history = createPulseHistory(MOBILE_CAPACITIES);
...
describe('pulse history consumers', () => {
  ...
  it('breaks equal veil contribution ties by newest start time then serial', () => {
    ...
    const sameTime = createPulseHistory(DESKTOP_CAPACITIES);
...
describe('pulse history snapshots and validation', () => {
  ...
  it('fails fast for unknown tiers, sources, invalid values, handles, and snapshots', () => {
    expect(() => createPulseHistory()).toThrow(/capacities/);
    expect(() => createPulseHistory({ systemCapacity: 0, userCapacity: 2 })).toThrow(/systemCapacity/);
    const history = createPulseHistory(DESKTOP_CAPACITIES);
    ...
    expect(() => restorePulseHistory(history, capturePulseHistory(createPulseHistory(MOBILE_CAPACITIES))))
      .toThrow(/capacities/);
    expect(() => lifeEnvelope(-1, 0)).toThrow(/age/);
  });

  it('projects desktop rings to the newest two entries by serial', () => {
    const desktop = createPulseHistory(DESKTOP_CAPACITIES);
    for (let serial = 1; serial <= 5; serial++) {
      insertPulse(desktop, pulse(serial % 2 ? 'auto' : 'system', 0, { originX: serial }));
    }
    for (let serial = 6; serial <= 8; serial++) {
      insertPulse(desktop, pulse('user', 0, { originX: serial }));
    }
    insertPulse(desktop, pulse('auto', 0, { originX: 9 }));
    insertPulse(desktop, pulse('system', 0, { originX: 10 }));

    const mobile = projectPulseHistory(desktop, MOBILE_CAPACITIES);
    expect(capturePulseHistory(mobile)).toMatchObject({
      systemCapacity: 2,
      userCapacity: 2,
      systemCursor: 0,
      userCursor: 0,
    });
    expect(getLivePulses(mobile).map((entry) => [entry.originX, entry.serial])).toEqual([
      [9, 9], [10, 10], [7, 7], [8, 8],
    ]);
    expect(insertPulse(mobile, pulse('system', 0, { originX: 11 })).serial).toBe(11);
  });

  it('projects mobile rings into desktop rings while leaving empty slots', () => {
    const mobile = createPulseHistory(MOBILE_CAPACITIES);
    insertPulse(mobile, pulse('auto', 0, { originX: 1 }));
    insertPulse(mobile, pulse('system', 0, { originX: 2 }));
    insertPulse(mobile, pulse('user', 0, { originX: 3 }));

    const desktop = projectPulseHistory(mobile, DESKTOP_CAPACITIES);
    const snapshot = capturePulseHistory(desktop);
    expect(snapshot.systemCursor).toBe(2);
    expect(snapshot.userCursor).toBe(1);
    expect(snapshot.systemSlots.slice(0, 2).map((slot) => slot.serial)).toEqual([1, 2]);
    expect(snapshot.systemSlots.slice(2)).toEqual([null, null, null]);
    expect(snapshot.userSlots[0].serial).toBe(3);
    expect(snapshot.userSlots.slice(1)).toEqual([null, null]);
    expect(insertPulse(desktop, pulse('system', 0, { originX: 4 })).serial).toBe(4);
    expect(getLivePulses(desktop).map((entry) => entry.originX)).toEqual([1, 2, 4, 3]);
  });

  it('rejects malformed and internally inconsistent snapshots and projections', () => {
    const history = createPulseHistory(MOBILE_CAPACITIES);
    insertPulse(history, pulse('auto'));
    insertPulse(history, pulse('user'));
    const captured = capturePulseHistory(history);
    const duplicateSerial = {
      ...captured,
      systemSlots: captured.systemSlots.map((slot) => (slot === null ? null : { ...slot })),
      userSlots: captured.userSlots.map((slot) => (slot === null ? null : { ...slot })),
    };
    duplicateSerial.userSlots[0].serial = duplicateSerial.systemSlots[0].serial;

    expect(() => restorePulseHistory(history, {
      ...captured,
      systemSlots: captured.systemSlots.slice(1),
    })).toThrow(/capacities/);
    expect(() => restorePulseHistory(history, duplicateSerial)).toThrow(/unique/);
    expect(() => restorePulseHistory(history, {
      ...captured,
      systemCursor: 0,
    })).toThrow(/valid ring/);
    expect(() => projectPulseHistory(history, { systemCapacity: 2, userCapacity: -1 }))
      .toThrow(/userCapacity/);
  });
});
