...
- 疑問: PR #45 の初回 Linux CI では、新設 value gate より前の既存 `qa:water` が8フレームの寸法不一致で停止した。artifact の初回 `.canvas-shell` は `862x724`、固定契約とローカル安定値は `862x735` だった。shader canvas の下にある telemetry は30サンプル到達まで `warming` 文言を表示し、Linux の monospace metrics では `live` への切替時に auto row の高さが変わるため、固定1200ms sleepだけでは低速CI上のレイアウト確定を保証できなかった。閾値・画像のcrop/resize・製品UIは変更せず、QAを telemetry `live` と loader消失まで待たせ、実測した canvas 寸法を8フレームで fail-fast 検証し、capture targetも水面そのものの `.shader-canvas` へ限定する。寸法不一致時は全寸法と最初のframe indexを報告する。
- 第2回 Linux CI は上記の `qa:water` を8/8フレーム `862x735`、seam `0.333` で通過し、根因と修正を実証した。その直後の `qa:water-value` は、Playwright Clock をpauseしたまま telemetryが `warming` の段階で固定寸法 `862x735` を待ってtimeoutした。Windowsでは warming/liveが同じ高さのため潜在していたが、Linuxではvirtual warm-upを実行しない限り30サンプルへ進まず、待ち条件が自己阻害になる。既存の `runFor(1200ms)` を寸法検査の前へ移し、telemetry `live` と固定CSS/backing寸法を同時に前提化する。取得開始時刻1328ms以降、64 frame hash、閾値、ROI、render settleは変更しない。
