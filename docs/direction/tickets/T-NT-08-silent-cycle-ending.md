...
- live ending 必须等待第 IX 章 transition 实际 commit 后才能推进。一次性 clock jump、低帧率 stall 或过早到达的 media `ended` 只能把 score clamp 在终点；后续 active frames 先提交 IX，再按顺序补齐 withdrawal/cues/finish，不能以当前 raw score 代替章节提交。
