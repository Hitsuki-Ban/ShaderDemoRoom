...
- `pnpm test` 20 files / 76 tests、`pnpm lint`、token lint、`pnpm typecheck`、`pnpm build`、exhibit sync、`git diff --check`: pass。synthetic 60/30/4 FPS、jitter/long frame、p95 nearest-rank、bucket gaps、reset、software/hardware/unknown classifier、Glass 19 calls、HUD native/external states を固定した。lavapipe / OpenSWR / WARP と未認識 unmasked renderer の負向 case も追加した。
...
- `pnpm qa:telemetry-reference`: 1440×900 Voxel default、5秒 warm-up + 15秒 measurement。bundled Chromium / SwiftShader median 15.37 FPS / 65.05 ms、system Chrome D3D11 / RTX 4070 Ti median 200 FPS / 5.00 ms、両環境とも15 sampleすべて 19 calls。同じ hardware renderer で T-SH-02 baseline build と source revision `0c1b17c` の T-SH-03 candidate build を5組交錯・交互順序で直接測定し、paired median regression 0.00% で5% gate を通過した。全 raw pair と renderer string は schema v1 JSON `docs/direction/captures/telemetry-reference-2026-07-18.json` に保存した。
