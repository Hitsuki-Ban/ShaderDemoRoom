...
- 独立 review 対応: glass shell の生成時 `0.03 + 0.03*(ior-1)` と更新時 `0.05 + 0.05*(ior-1)` の二重曲線を、生成時の低い曲線へ単一化した。dispersion / roughness / thickness の更新では opacity が不変、IOR 更新だけが同じ関数を通る回帰テストを追加した。これにより初回操作時の不連続な明度ジャンプで材質 dispersion が覆われない。
- QA: `qa:glass` は16 capture / 8 ON-OFF payoff state とし、4材質比較、IOR=1 pixel identity、roughness、16/15 calls、spread/IOR caustics、静止/運動、180-frame allocation を恒常 gate にした。T-GO-03 の cross-IOR 契約は dispersion=0 の白色 beam control で検証する。従来の P99.9 は IOR とともに positive footprint が広がるほど tail の採取数も増えるため、各状態で strongest 32 pixels を同数比較する `focusCoreLinearMean` を追加・単体テストし、**0.590 → 0.677 → 0.734** の単調増加を確認した。採用 dispersion の default/high IOR も通常の presence/shape/coverage/position/clipping gate をすべて通し、P99.9 は **0.585 / 0.523**。spread 半値半径は **8.65 → 10.06 → 13.37**、材質 chroma gain は **2.211 → 2.561**、`>5` coverage は **11.74% → 16.67%**、IOR=1差分0、all-channel clip 0、drag calls `16→16` / geometries `24→24` / 禁止allocation 0、browser error 0。
...
- 証拠: `captures/t-go-05-default-zero.png`、`t-go-05-default-adopted.png`、`t-go-05-high-ior-zero.png`、`t-go-05-high-ior-adopted.png`、`t-go-05-roughness-0-55.png`、`t-go-05-glass-qa-2026-07-20.json` (`recordedAt: 2026-07-20T14:43:54.251Z`)、`t-go-05-telemetry-2026-07-20.json`。
- 検証: `pnpm test` (34 files / 269 tests)、`pnpm lint`、`pnpm typecheck`、`pnpm build`、`pnpm exhibits:check`、`pnpm qa:visual`、`pnpm qa:glass` を通過。`pnpm qa:renderer` は未変更の Voxel Water software FPS が本機の歴史的 gate を下回り2回失敗(mean `13.96` / `13.42`)したが、同じrunの Glassは16 callsを維持し、本票の独立hardware paired gateは上記のとおり通過した。
