...
- `src/rooms/glass-optics/runtime.ts:414-435` は `disposeObject(root)` の traverse で全ジオメトリ・マテリアルを破棄した後、15 マテリアルの明示リストを再度 dispose する。three.js の dispose は冪等だが、2つの所有リストが将来の追加/削除でドリフトする。
- root 外の PMREM 資産 `environment.texture` / `pmrem`(`runtime.ts:433-434`)は traverse では回収できず、明示的な所有権が必要。
...
`disposeObject(root)` を root 配下の Geometry / Material を解放する唯一の経路にする。重複する明示 material リストを削除し、root 外で所有する `environment.texture` と `pmrem` だけを明示 dispose する。シーン実体は必ず root 配下へ追加する不変条件を runtime の所有境界に記す。
