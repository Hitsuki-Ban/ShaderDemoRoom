...
- 依存: なし(独立着手可)。T-GO-03(コースティクス)とは床の明度設計を共有するため、値設計の合意だけ先に取ること。T-GO-06 はダークフィールド env の完成を前提とするため本チケットが先行
...
2. **床を「暗い反射床」へ**: 段階 1(必須)— 床マテリアルを低 roughness・高 metalness 側へ振り、ストリップ env の写り込みで「磨かれた黒床」を成立させる(追加パスなしのフェイク反射。ポストプロセスなし制約を維持)。アルベドは現行 0x121c26 からさらに落とし、コースティクスとビーム着地点が床の最輝要素になる値関係を作る。段階 2(任意)— 実写り込み(Reflector 系の平面反射 1 パス)は描画コストがシーン 1 回分増えるため、**T-GO-01 の実 GPU 基準値取得後に FPS 予算と environment 分類(software では無効化)を条件に検討**する。本チケットでは判断記録のみ必須。
...
4. **背景の分離**: 注視点背後に微かなラジアルグラデーション(中心がわずかに明るい)を導入する(背景平面 1 枚のシェーダまたは焼きテクスチャ。research §2.9「グラデーション背景」)。ガラスの暗部形状を背景から分離する。
...
- **数値基準**: telemetry(`data-telemetry-json`)で FPS が現行値から ±5% 以内(env 差し替え・床マテリアル変更は描画コスト中立のはず)。draw calls は増加を 2 以内に収める(背景平面の追加など)。
...
- **決定論的キャプチャ**: root sway(`runtime.ts:411`)が生きている間はスクリーンショットに位相差が乗る。比較キャプチャは入場後固定待機時間で撮る(現行 QA と同条件)。sway の扱い自体は T-GO-07 で解消する。
