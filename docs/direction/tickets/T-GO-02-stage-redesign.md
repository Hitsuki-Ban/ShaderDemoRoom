...
- 安定性/性能: `Light X` の連続 drag 前後で 16 calls / 25 geometries を維持し、禁止 allocation 0、console error 0。`b63b3dd` との 5 組 interleaved production 比較の paired median は **-0.83%**（許容上限 +5%）。hardware reference は 163.36 FPS。
